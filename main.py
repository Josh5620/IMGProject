...
def draw_ammo_counter(player):
    """Draw ammunition counter at top of screen"""
    font = pygame.font.Font(None, 36)
    ammo_text = font.render(f"Ammo: {player.current_ammo}/{player.max_ammo}", True, (255, 255, 255))
    
    # Add background for better visibility
    text_rect = ammo_text.get_rect()
    bg_rect = pygame.Rect(10, 50, text_rect.width + 10, text_rect.height + 5)
    pygame.draw.rect(screen, (0, 0, 0, 128), bg_rect)
    
    screen.blit(ammo_text, (15, 52))
    
    # Draw cooldown indicator
    if player.shooting_cooldown > 0:
        cooldown_width = int((player.shooting_cooldown / player.cooldown_time) * 100)
        cooldown_rect = pygame.Rect(15, 85, cooldown_width, 8)
        pygame.draw.rect(screen, (255, 255, 0), cooldown_rect)

def draw_powerup_status(player):
    ...
    if player.powerup_timers["speed"] > 0:
        time_left = player.powerup_timers["speed"] / 60  # Convert to seconds
        speed_text = font.render(f"Speed Boost: {time_left:.1f}s", True, (0, 255, 0))
        screen.blit(speed_text, (15, y_offset))
        y_offset += 25
    
    # Damage boost
    if player.powerup_timers["damage"] > 0:
        time_left = player.powerup_timers["damage"] / 60
        damage_text = font.render(f"Damage Boost: {time_left:.1f}s", True, (255, 255, 0))
        screen.blit(damage_text, (15, y_offset))
        y_offset += 25
    
    # Shield
    if player.powerup_timers["shield"] > 0:
        time_left = player.powerup_timers["shield"] / 60
        shield_text = font.render(f"Shield: {time_left:.1f}s", True, (0, 0, 255))
